...
export interface DashboardStorePort {
  listProjects(query?: string, workspaceId?: string): Promise<NativeProjectSnapshot[]>;
  ...
  listProjectJobs(projectId: string, workspaceId?: string): Promise<NativeJob[]>;
  submitJob(input: NativeJobSubmitInput): Promise<NativeJob>;
  getProjectEventsSince(projectId: string, lastSeq: number, workspaceId?: string): Promise<NativeProjectEvent[]>;
}
