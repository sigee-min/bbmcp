...
export const normalizeOptionalWorkspaceId = (value: unknown): string | undefined => {
  if (typeof value !== 'string') {
    return undefined;
  }
  const trimmed = value.trim();
...
const parseHeaderString = (value: unknown): string => {
  ...
  if (Array.isArray(value)) {
    return parseHeaderString(value[0]);
  }
  return '';
};

export const resolveActorContext = (headers: Record<string, unknown>): GatewayActorContext => {
...
export const errorMessageOrFallback = (error: unknown, fallback: string): string =>
  error instanceof Error ? error.message : fallback;

export const forbiddenPlan = (message: string, code = 'forbidden'): ResponsePlan =>
  jsonPlan(403, {
    ok: false,
    code,
    message
...
export const workspaceNotFoundPlan = (workspaceId: string): ResponsePlan =>
  jsonPlan(404, {
    ok: false,
...
export const projectLoadFailedPlan = (projectId: string): ResponsePlan =>
  jsonPlan(404, {
    ok: false,
    code: 'project_load_failed',
