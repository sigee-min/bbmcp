...
## 키스페이스 격리 모델
- 워크스페이스 API 키는 워크스페이스 툴만 `tools/list`에서 보입니다.
- 서비스 API 키는 서비스 관리 툴만 `tools/list`에서 보입니다.
- 다른 키스페이스 전용 툴을 이름으로 추측 호출해도 `unknown_tool`만 반환되며, 툴 존재 여부는 노출되지 않습니다.
- 이미 열린 세션에 다른 API 키를 재사용하면 인증 실패(401)로 차단됩니다.
