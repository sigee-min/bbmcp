...
- Track B: server deployment (`apps/web` + `apps/mcp-gateway` + `apps/worker`)

## Executable Boundaries

- Plugin bundle entry: `apps/plugin-desktop/src/index.ts`
- Headless/sidecar entry: `apps/ashfox/src/index.ts`
- MCP gateway entry: `apps/mcp-gateway/src/index.ts`
...
2. Route backend selection through `packages/backend-core` interfaces only.
